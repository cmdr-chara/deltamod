...
This gate applies to the unsigned beta only. The existing Electron release remains the rollback artifact until every check below is green.
...
9. Windows x64: install and uninstall the NSIS beta, verify no Authenticode signature is present, run a mod import and CSX patch smoke test, and verify all five sidecars.
...
11. macOS x64 and arm64: open the DMG, verify the app bundle architecture, run the same persistence/protocol smoke tests, verify G3MTool, and mark UndertaleModTool CSX unavailable on arm64 rather than silently falling back.
12. Compare the Tauri smoke results against the Electron beta on the same fixture set. Any changed result is a release blocker unless documented in the release notes.
...
2. If any gate fails after publication, mark the Tauri assets as withdrawn in the GitHub prerelease notes and stop Tauri auto-distribution.
...
5. If a user installed the Tauri beta, direct them to uninstall it and install the prior Electron beta; user data must remain in the documented application data directory and must not be deleted by uninstall.
