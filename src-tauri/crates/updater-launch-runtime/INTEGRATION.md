...
Construct `UpdaterGate` with `UpdaterGate::configured`. It remains unsupported unless the app is packaged, updater artifacts are enabled, every configured endpoint is HTTPS, and a public key is present. The current shell has `bundle.active: false` and no updater configuration, so its truthful gate is unsupported; do not substitute a successful no-op adapter.
...
2. Set `bundle.createUpdaterArtifacts` to `true` when distributable bundles are enabled.
3. Configure `plugins.updater.pubkey` with the release public key and at least one HTTPS endpoint.
4. Publish Tauri's signed updater artifact and matching signature in the endpoint JSON for each target.
5. Add only the updater permissions required by the chosen Rust or frontend integration.
