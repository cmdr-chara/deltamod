...
Construct `UpdaterGate` with `UpdaterGate::configured`. It remains unsupported unless the app is packaged, updater artifacts are enabled, every configured endpoint is HTTPS, and a public key is present. The current stable shell has `bundle.active: true`, `bundle.createUpdaterArtifacts: false`, and no updater plugin configuration, so its truthful gate remains unsupported. Do not substitute a successful no-op adapter. `scripts/verify-secure-updater-config.js` is the repository policy gate for any future enablement and must continue to pass.
...
Register `fireUpdate`, `start-update`, `ignore-update`, and optionally `updater-status` in the channel router. Store the updater behind application-owned synchronization so checks and installs cannot overlap.

Host wiring still required before changing the gate to supported:

1. Add `tauri-plugin-updater = "2"` and initialize `tauri_plugin_updater::Builder::new().build()`.
2. Set `bundle.createUpdaterArtifacts` to `true` only in the same reviewed change that supplies the remaining signed-update configuration.
...
6. Add only the updater permissions required by the chosen Rust or frontend integration.
