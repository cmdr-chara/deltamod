...
- `modSources:clearNexusKey` cancels active SSO, clears `CredentialKind::NexusSsoKey`, and returns `true` only after keyring deletion succeeds.
...
- Nexus status/download operations load the Nexus key only for the immediate native HTTPS request.
- Successful restricted GameBanana login and validated Nexus SSO are the only writers.
